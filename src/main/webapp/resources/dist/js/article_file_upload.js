...
function deleteFileWrtPage(that) {
    var url = "/myboard/file/delete";
    deleteFile(url, that);
}

// 파일 삭제(수정페이지) : 서버에 저장된 첨부파일과 DB에 저장된 첨부파일 정보 삭제처리
function deleteFileModPage(that, article_no) {
    var url = "/myboard/file/delete/" + article_no;
    deleteFile(url, that);
}
...
function getFileInfo(fullName) {
    ...
    if (checkImageType(fullName)) {
        imgSrc = "/myboard/file/display?fileName=" + fullName; // 썸네일 이미지 링크
        uuidFileName = fullName.substr(14);
        var originalImg = fullName.substr(0, 12) + fullName.substr(14);
        // 원본 이미지 요청 링크
        originalFileUrl = "/myboard/file/display?fileName=" + originalImg;
    } else {
        imgSrc = "/resources/upload/files/file-icon.png"; // 파일 아이콘 이미지 링크
        uuidFileName = fullName.substr(12);
        // 파일 다운로드 요청 링크
        originalFileUrl = "/myboard/file/display?fileName=" + fullName;
    }
